    ...
    <div className="mx-auto max-w-screen-xl w-full">
      <div className="grid lg:grid-cols-2 pb-4 sm:py-6 lg:py-12 gap-4 sm:gap-6">
        <div className="flex flex-col gap-2 sm:gap-4 overflow-hidden">
          <Skeleton
            className="w-full aspect-square rounded-none sm:rounded-md"
          />
          <div className="flex items-center gap-2 sm:space-x-4 min-w-max mx-2 sm:mx-0">
            {Array.from({ length: 7 }).map((_, idx) => (
              <Skeleton
                key={idx}
                className="size-20 rounded-none sm:rounded-md"
              />
            ))}
          </div>
        </div>
        <div className="flex flex-col gap-1.5 sm:gap-2 px-2 sm:px-4">
