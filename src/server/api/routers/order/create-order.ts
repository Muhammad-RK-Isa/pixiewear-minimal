...
export async function createOrder(ctx: ProtectedContext, input: CreateOrderSchemaType) {
  if (ctx.user.phone !== input.phone) {
    const isUserVerified = await ctx.db.query.orders.findFirst({
      where: (t, { and, eq }) =>
        and(
          eq(t.phone, input.phone),
          eq(t.userVerified, true)
        )
    }).then((od) => !!od)
    if (!isUserVerified) {
      if (input.verificationCode) {
        const validVerificationCode = await ctx.db.query.securityCodes.findFirst({
          where: (t, { and, eq, gte }) =>
            and(
